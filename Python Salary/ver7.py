...
def tinh_luong_theo_gio():
    try:
        ...
        if so_gio_lam_moi_ngay > 0:
            ...
            luong_theo_gio_hien_thi.config(text=f"{luong_theo_gio:,.3f} VNĐ/giờ")
            cap_nhat_khau_tru_bao_hiem(luong_co_ban) # Cập nhật khấu trừ bảo hiểm khi lương cơ bản thay đổi
        else:
            luong_theo_gio_hien_thi.config(text="Nhập số giờ làm việc hợp lệ.")
            khau_tru_bao_hiem_value.config(text="0.000 VNĐ") # Reset khấu trừ bảo hiểm
    except ValueError:
        luong_theo_gio_hien_thi.config(text="")
        khau_tru_bao_hiem_value.config(text="0.000 VNĐ") # Reset khấu trừ bảo hiểm

def cap_nhat_khau_tru_bao_hiem(luong_co_ban):
    try:
        luong_co_ban_bh = float(luong_co_ban)
        khau_tru_bao_hiem = luong_co_ban_bh * 0.105
        khau_tru_bao_hiem_value.config(text=f"{khau_tru_bao_hiem:,.3f} VNĐ")
    except ValueError:
        khau_tru_bao_hiem_value.config(text="0.000 VNĐ")

def tinh_tong_luong():
    tong_tien_luong_truoc_khau_tru = 0
    thong_tin_luong_chi_tiet.config(text="") # Xóa thông tin chi tiết cũ
    thong_bao_loi.config(text="") # Xóa thông báo lỗi

    if tinh_luong_cb_var.get():
        try:
            luong_co_ban = float(luong_co_ban_entry.get())
            tong_tien_luong_truoc_khau_tru += luong_co_ban
            thong_tin_luong_chi_tiet.config(text=thong_tin_luong_chi_tiet.cget("text") + f"Lương cơ bản: {luong_co_ban:,.3f} VNĐ\n")
        except ValueError:
    ...
    if tinh_luong_tc_var.get():
        try:
            ...
            tong_tien_luong_truoc_khau_tru += luong_tro_cap
            thong_tin_luong_chi_tiet.config(text=thong_tin_luong_chi_tiet.cget("text") + f"Lương trợ cấp: {luong_tro_cap:,.3f} VNĐ\n")
        except ValueError:
        ...
        try:
            ...
            for frame in tang_ca_frames:
                try:
                    ...
                    pass
            tong_tien_luong_truoc_khau_tru += tong_luong_lam_them
            thong_tin_luong_chi_tiet.config(text=thong_tin_luong_chi_tiet.cget("text") + f"Lương làm thêm: {tong_luong_lam_them:,.3f} VNĐ\n")
        except ValueError:
    ...
    try:
        khau_tru_thue = float(khau_tru_thue_entry.get())
        tong_khau_tru += khau_tru_thue
        thong_tin_luong_chi_tiet.config(text=thong_tin_luong_chi_tiet.cget("text") + f"Khấu trừ thuế: {khau_tru_thue:,.3f} VNĐ\n")
    except ValueError:
        thong_bao_loi.config(text="Lỗi: Vui lòng nhập số hợp lệ cho khấu trừ thuế.")
        return

    try:
        luong_co_ban_for_bh = float(luong_co_ban_entry.get())
        khau_tru_bao_hiem = luong_co_ban_for_bh * 0.105
        tong_khau_tru += khau_tru_bao_hiem
        thong_tin_luong_chi_tiet.config(text=thong_tin_luong_chi_tiet.cget("text") + f"Khấu trừ bảo hiểm (10.5%): {khau_tru_bao_hiem:,.3f} VNĐ\n")
    except ValueError:
        thong_bao_loi.config(text="Lỗi khi tính toán khấu trừ bảo hiểm.")
        return

    try:
        khau_tru_cong_doan = float(khau_tru_cong_doan_entry.get())
        tong_khau_tru += khau_tru_cong_doan
        thong_tin_luong_chi_tiet.config(text=thong_tin_luong_chi_tiet.cget("text") + f"Khấu trừ phí công đoàn: {khau_tru_cong_doan:,.3f} VNĐ\n")
    except ValueError:
        thong_bao_loi.config(text="Lỗi: Vui lòng nhập số hợp lệ cho khấu trừ phí công đoàn.")
        return

    try:
        khau_tru_khac = float(khau_tru_khac_entry.get())
        tong_khau_tru += khau_tru_khac
        thong_tin_luong_chi_tiet.config(text=thong_tin_luong_chi_tiet.cget("text") + f"Khấu trừ khác: {khau_tru_khac:,.3f} VNĐ\n")
    except ValueError:
        thong_bao_loi.config(text="Lỗi: Vui lòng nhập số hợp lệ cho khấu trừ khác.")
        return

    tong_tien_luong_sau_khau_tru = tong_tien_luong_truoc_khau_tru - tong_khau_tru
    ket_qua_label.config(text=f"Tổng tiền lương: {tong_tien_luong_sau_khau_tru:,.3f} VNĐ")
...
def them_khung_tang_ca():
    new_tang_ca_frame = ttk.Frame(tang_ca_canvas) # Đặt khung mới vào canvas
    ...
    tang_ca_canvas.config(scrollregion=tang_ca_canvas.bbox("all")) # Cập nhật vùng cuộn
    tang_ca_canvas.yview_moveto(1.0) # Tự động cuộn xuống dưới
...
# Khung nhập tăng ca (sử dụng Canvas với thanh cuộn)
tang_ca_frame = ttk.LabelFrame(root, text="Thông tin tăng ca")
tang_ca_frame.pack(padx=10, pady=10, fill="both", expand=True)

tang_ca_canvas = tk.Canvas(tang_ca_frame)
tang_ca_canvas.pack(side="left", fill="both", expand=True)

tang_ca_scrollbar = ttk.Scrollbar(tang_ca_frame, orient="vertical", command=tang_ca_canvas.yview)
tang_ca_scrollbar.pack(side="right", fill="y")

tang_ca_canvas.configure(yscrollcommand=tang_ca_scrollbar.set)
tang_ca_canvas.bind('<Configure>', lambda e: tang_ca_canvas.config(scrollregion = tang_ca_canvas.bbox('all')))

tang_ca_frames = []
them_khung_tang_ca() # Thêm một khung ban đầu

them_button = ttk.Button(tang_ca_frame, text="+ Thêm giờ tăng ca", command=them_khung_tang_ca)
them_button.pack(pady=5)

# Khung các khoản khấu trừ
khau_tru_frame = ttk.LabelFrame(root, text="Các khoản khấu trừ")
khau_tru_frame.pack(padx=10, pady=10, fill="x")

# Khấu trừ thuế
khau_tru_thue_label = ttk.Label(khau_tru_frame, text="Khấu trừ thuế:")
khau_tru_thue_label.grid(row=0, column=0, padx=5, pady=5, sticky="w")
khau_tru_thue_entry = ttk.Entry(khau_tru_frame)
khau_tru_thue_entry.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
khau_tru_thue_entry.insert(0, "0") # Giá trị mặc định

# Khấu trừ bảo hiểm (tự động tính)
khau_tru_bao_hiem_label = ttk.Label(khau_tru_frame, text="Khấu trừ bảo hiểm (10.5%):")
khau_tru_bao_hiem_label.grid(row=1, column=0, padx=5, pady=5, sticky="w")
khau_tru_bao_hiem_value = ttk.Label(khau_tru_frame, text="0.000 VNĐ")
khau_tru_bao_hiem_value.grid(row=1, column=1, padx=5, pady=5, sticky="ew")

# Khấu trừ phí công đoàn
khau_tru_cong_doan_label = ttk.Label(khau_tru_frame, text="Khấu trừ phí công đoàn:")
khau_tru_cong_doan_label.grid(row=2, column=0, padx=5, pady=5, sticky="w")
khau_tru_cong_doan_entry = ttk.Entry(khau_tru_frame)
khau_tru_cong_doan_entry.grid(row=2, column=1, padx=5, pady=5, sticky="ew")
khau_tru_cong_doan_entry.insert(0, "40000") # Giá trị mặc định

# Khấu trừ khác
khau_tru_khac_label = ttk.Label(khau_tru_frame, text="Khấu trừ khác:")
khau_tru_khac_label.grid(row=3, column=0, padx=5, pady=5, sticky="w")
khau_tru_khac_entry = ttk.Entry(khau_tru_frame)
khau_tru_khac_entry.grid(row=3, column=1, padx=5, pady=5, sticky="ew")
khau_tru_khac_entry.insert(0, "0") # Giá trị mặc định
...
khau_tru_frame.columnconfigure(1, weight=1)
